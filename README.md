...
На разработку было затрачено всего ~10 часов, более подробно процесс разработки и размышлений можно посмотреть в [[TODO.md]] и истории коммитов.
...
* При запросе прогноза на конкретное время (например, `14:15`), сервис отправляет данные по местному для запрошенной локации времени. Выбираются данные по наиболее близкие по времени к сохраненным данным. 

### 2. Пользовательская система

**Проблема**: В задании указано, что методы 2, 3 и 4 должны принимать ID пользователя, но погодные данные одинаковы для всех пользователей. В требованиях не описано, каким именно образом ID пользователя влияет на предоставляемые данные.
**Решение**: Реализована система связей "многие-ко-многим". Город сохраняется в БД один раз, но привязывается к конкретным пользователям. Метод получения списка городов (`GET /cities`) фильтрует выдачу по `user_id`, если он передан.
